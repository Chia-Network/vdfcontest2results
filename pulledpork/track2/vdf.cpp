...
mpz_t D, L;
mpz_t dx, dy, By, Dy, x, y, bx, by, ax, ay, q, t, Q1, denom;
form F, f_;
...
inline void gmp_nudupl(form& f) {
    // gcd(A,B) = 1, so we can just toss the result here (tmp bx) and use G=1
    mpz_gcdext(bx, y, NULL, f.b, f.a);
...
int main(int argc, char* argv[]) {
    ...
    mpz_inits(D, L, NULL);
    mpz_inits(dx, dy, By, Dy, x, y, bx, by, ax, ay, q, t, Q1, denom, NULL);
    ...
    cout << F.a << endl << F.b;
    cout << flush;
    _exit(0); // all done, just quick terminate
}
