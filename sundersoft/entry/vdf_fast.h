...
void repeated_square_fast_work(bool is_slave, uint64 iterations) {
...
uint64 repeated_square_fast_multithread(form& f, const integer& D, const integer& L, uint64 iterations) {
    master_counter.reset();
    slave_counter.reset();

    square_state.init(D, L, f.a, f.b);
    memory_barrier();

    thread slave_thread(repeated_square_fast_work, false, iterations);

    repeated_square_fast_work(true, iterations);
...
uint64 repeated_square_fast_single_thread(form& f, const integer& D, const integer& L, uint64 iterations) {
    master_counter.reset();
    slave_counter.reset();
...
uint64 repeated_square_fast(form& f, const integer& D, const integer& L, uint64 iterations) {
    if (enable_threads) {
        return repeated_square_fast_multithread(f, D, L, iterations);
    } else {
        return repeated_square_fast_single_thread(f, D, L, iterations);
    }
}