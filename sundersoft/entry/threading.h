...
void* alloc_cache_line(size_t bytes) {
    ...
    void* res=aligned_alloc(64, aligned_bytes);
...
template<int d_expected_size, int d_padded_size> struct alignas(64) mpz : public mpz_base {
    ...
    uint64* modify_limbs(int num) {
        ...
        uint64* res=mpz_limbs_modify(*this, num);
...
thread_counter master_counter;
thread_counter slave_counter;
...
struct thread_state {
    ...
    thread_counter& this_counter() {
        return (is_slave)? slave_counter : master_counter;
    }

    thread_counter& other_counter() {
        return (is_slave)? master_counter : slave_counter;
    }
